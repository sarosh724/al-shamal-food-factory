...
<html lang="en">
	<head>
		...
		<title>{{__('al_shamal_food_factory')}}</title>
				...
				<div class="container">
					<div class="row justify-content-center py-2">
						<div class="col-9 col-md-12 text-center">
							<p class="text-color-light mb-0">{!!strip_tags(fetchPage('announcement-bar')->description_arabic)!!}</p>
						</div>
					</div>
				</div>
			</div>
											...
											<ul class="nav nav-pills position-relative">
												<li class="nav-item d-none d-sm-block">
													<span class="d-flex align-items-center font-weight-medium ws-nowrap text-3 ps-0">
														<a href="mailto:{{getSetting('email')->value}}" class="text-decoration-none text-color-dark text-color-hover-primary">
															{{getSetting('email')->value}}
												...
												<li class="nav-item nav-item-right-border nav-item-left-border-remove nav-item-left-border-sm-show">
													<span class="d-flex align-items-center font-weight-medium text-color-dark ws-nowrap text-3">
														<i class="icons icon-clock font-weight-bold position-relative text-3 top-1 me-2  px-2"></i>
														 {{getSetting('working-days')->value_arabic}}</span>
													</span>
												</li>
											</ul>
										</div>
										...
										<div class="header-nav-main header-nav-main-arrows header-nav-main-dropdown-no-borders header-nav-main-effect-3 header-nav-main-sub-effect-1">
											<nav class="collapse">
												<ul class="nav nav-pills" id="mainNav">
													<li><a href="{{route('index')}}" class="nav-link {{is_active_menu(route('index'))}}">{{__('home')}}</a></li>
													<li><a href="{{route('about')}}" class="nav-link {{is_active_menu(route('about'))}}">{{__('about')}}</a></li>
													<li class="dropdown">
														<a href="{{route('services')}}" class="nav-link {{is_active_menu(route('services'))}} dropdown-toggle">{{__('services')}}</a>
													...
													<li><a href="{{route('products')}}" class="nav-link {{is_active_menu(route('products'))}}">{{__('products')}}</a></li>
													<li><a href="{{route('contact-us')}}" class="nav-link {{is_active_menu(route('contact-us'))}}">{{__('contact')}}</a></li>
												</ul>
											</nav>
										</div>
									</div>
									<ul class="header-extra-info custom-left-border-1 d-none d-xl-block">
										<li class="d-none d-sm-inline-flex ms-0">
											...
											<div class="header-extra-info-text line-height-2">
												<span class="text-1 font-weight-semibold text-color-default">{{__('call_us_now')}}</span>
												<strong class="text-4" style="direction: ltr;"><a href="tel:{{ getSetting('contact-1')->value }}" class="text-color-hover-primary text-decoration-none">{{ getSetting('contact-1')->value }}</a></strong>
											</div>
										</li>
									</ul>
