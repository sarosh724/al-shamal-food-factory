        ...
        <div class="container">
            <div class="row">
                <div class="col-md-8 order-2 order-md-1 align-self-center p-static">
                    <h1 class="text-dark"> <strong>{{__('products')}}</strong></h1>
                    <span class="sub-title text-dark">{{ $productBreadcrumb->subtitle_arabic }}</span>
                </div>
    ...
    <div class="shop container py-4 my-5">
        <div class="row align-items-center justify-content-between mb-4">
            <div class="col-auto mb-3 mb-md-0 order-md-2">
                <div class="d-flex align-items-center">
                    <span>{{__('showing')}} {{ $products->firstItem() }} - {{ $products->lastItem() }} of
            ...
            <div class="col-md-6 order-md-1">
                <form method="get" action="{{ route('products') }}">
                    <div class="custom-select-1" style="max-width: 280px;">
                        <select name="service" class="form-select form-control border px-3 py-2 h-auto" style="direction: ltr;"
                            onchange="this.form.submit()">
                            <option value="all">{{__('sort_by_services')}}</option>
                            @foreach (fetchServices() as $service)
                                <option value="{{ $service->id }}" {{ $serviceId == $service->id ? 'selected' : '' }}>{{ Str::title($service->title_arabic) }}</option>
                            @endforeach
                        </select>
                    </div>
                </form>
                        ...
                        <div class="product-thumb-info mb-3">
                            ...
                            <a href="javascript:;">
                                <div class="product-thumb-info-image bg-light">
                                    <img alt="{{ $product->title_arabic }}" class="img-fluid"
                        ...
                        <div class="d-flex justify-content-between">
                            <div>
                                ...
                                <h3
                                    ...
                                    <a href=""
                                        class="text-color-dark text-color-hover-primary">{{ $product->title_arabic }}</a>
                                </h3>
                            </div>
                        </div>
        ...
        <div class="row align-items-center justify-content-md-between mt-4">
            <div class="col-md-auto mb-3 mb-md-0">
                <p class="mb-0">{{__('showing')}} {{ $products->firstItem() }} - {{ $products->lastItem() }} of
                    {{ $products->total() }} {{__('products')}}</p>
            </div>
